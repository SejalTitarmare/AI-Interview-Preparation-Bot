...
# ── READ SPEECH/ANSWER FROM URL PARAMS ──
qp = st.query_params
...
if st.session_state.mode is None:
    ...
    # ── 4 columns: Practice | Quiz | Idea Generator | Course Module ──
    ...
    with col1:
        ...
        """, unsafe_allow_html=True)
        st.markdown(" ")
        if st.button("▶ Start Practice Mode", type="primary",
                     use_container_width=True, key="btn_practice"):
    ...
    with col2:
        ...
        """, unsafe_allow_html=True)
        st.markdown(" ")
        if st.button("▶ Start Quiz Mode", type="primary",
    ...
    with col3:
        ...
        """, unsafe_allow_html=True)
        st.markdown(" ")
        if st.button("▶ Start Idea Generator", type="primary",
                     use_container_width=True, key="btn_ideas"):
            st.session_state.mode = "ideas"
            st.rerun()

    # ── NEW: Course Module Ideas ──────────────────────────────────────
    with col4:
        st.markdown("""
        <div style="
            ...
            border-radius:15px; padding:28px;
            min-height:300px;
            background:linear-gradient(135deg,#fa823111,#f7b73111);">
            <h2 style="color:#fa8231; margin-top:0;">📚 Course Module Ideas</h2>
            <p>Company uploads course syllabus PDF → student picks which
            module they finished → gets <b>3 project ideas</b> using
            ONLY tools taught in that module.</p>
            <ul>
                <li>Company uploads PDF syllabus</li>
                <li>Auto-extracts all modules with Groq</li>
                <li>Student selects completed module</li>
                <li>Ideas match exactly what they know</li>
            </ul>
        </div>
        """, unsafe_allow_html=True)
        st.markdown(" ")
        if st.button("▶ Start Course Mode", type="primary",
                     use_container_width=True, key="btn_course"):
...
if st.session_state.mode == "quiz":
    ...
    with st.sidebar:
        ...
        answer_mode = st.radio("How do you want to answer?",
                               ["⌨️ Type", "🎤 Speak"], index=0)
    ...
    if not st.session_state.quiz_started:
        ...
        if st.session_state.auto_speak and not st.session_state.answer_submitted:
            q_text = question_data['question'].replace('"', "'").replace('`', '')
        ...
        if not st.session_state.answer_submitted:
            ...
            if st.session_state.answer_mode == "speak":
                ...
                <div style="font-family:Arial; padding:10px; text-align:center;">
                    <button id="mic-btn" onclick="toggleMic()" style="background:#ff4b4b; color:white; border:none; padding:12px 24px; border-radius:50px; font-size:16px; cursor:pointer; width:220px; margin:6px;">🎤 Start Speaking</button>
                    <div id="status" style="background:#f0f2f6; border-radius:8px; padding:10px; font-size:14px; color:#555; margin:8px auto; max-width:520px;">Click mic and speak your answer</div>
                    <div style="text-align:left; max-width:520px; margin:0 auto 6px auto; font-size:13px; color:#888;">📝 Recognized text:</div>
                    <div id="transcript" style="background:white; border:2px solid #ddd; border-radius:10px; padding:12px; font-size:15px; color:#333; min-height:80px; max-width:520px; margin:0 auto 12px auto; text-align:left; white-space:pre-wrap;">(nothing heard yet)</div>
                    <button onclick="useAnswer()" style="background:#28a745; color:white; border:none; padding:12px 24px; border-radius:50px; font-size:16px; cursor:pointer; width:220px; margin:6px;">✅ Use This Answer</button>
                </div>
                <script>
                ...
                function startMic(){
                    ...
                    if(!API){ setStatus('❌ Use Chrome!','#c62828','#ffebee'); return; }
                    recog=new API(); recog.lang='en-US'; recog.continuous=true; recog.interimResults=true;
                    ...
                    recog.onresult=function(e){ finalText=''; var interim='';
                        for(var i=0;i<e.results.length;i++){
                            if(e.results[i].isFinal) finalText+=e.results[i][0].transcript+' ';
                            else interim+=e.results[i][0].transcript;
                        }
                        document.getElementById('transcript').innerText=(finalText+interim).trim();
                    };
                    recog.onerror=function(e){ listening=false; resetMic(); setStatus('❌ Error: '+e.error,'#c62828','#ffebee'); };
                    recog.onend=function(){ listening=false; resetMic();
                        if(finalText.trim()) setStatus('✅ Done! Click "Use This Answer"','#1b5e20','#e8f5e9');
                    };
                    recog.start();
                }
                function stopMic(){ if(recog) recog.stop(); listening=false; resetMic(); }
                function resetMic(){ document.getElementById('mic-btn').style.background='#ff4b4b'; document.getElementById('mic-btn').innerText='🎤 Start Speaking'; }
                function setStatus(msg,color,bg){ var b=document.getElementById('status'); b.innerText=msg; b.style.color=color; b.style.background=bg; }
                function useAnswer(){
                    var text=finalText.trim()||document.getElementById('transcript').innerText.trim();
                    if(!text||text==='(nothing heard yet)'||text.length<2){ setStatus('❌ Speak first!','#c62828','#ffebee'); return; }
                    if(listening){ stopMic(); }
                    setStatus('🔎 Filling answer box...','#0d47a1','#e3f2fd');
                    setTimeout(function(){
                        try {
                            var areas=window.parent.document.querySelectorAll('textarea'); var filled=false;
                            for(var i=0;i<areas.length;i++){
                                ...
                                if(area.placeholder && area.placeholder.indexOf('Use This Answer')!==-1){
                                    ...
                                    area.focus(); filled=true; break;
                                }
                            }
                            if(filled){ setStatus('✅ Answer box filled! Edit if needed, then Submit.','#1b5e20','#e8f5e9'); }
                            else{ setStatus('⚠️ Could not find answer box. Please type manually.','#c62828','#ffebee'); }
                        } catch(err){ setStatus('⚠️ Could not fill automatically. Please type manually.','#c62828','#ffebee'); }
                    }, 150);
                }
                </script>
                """
                ...
                user_answer = st.text_area("answer_area",
                    value=st.session_state.recognized_answer, height=130,
                    placeholder="Speak above → click Use This Answer → text appears here. Or type directly.",
                    label_visibility="collapsed", key="quiz_speech_answer")
            else:
                user_answer = st.text_area("Type your answer here:", height=150,
                    placeholder="Type your answer in detail...", key="quiz_type_answer")
            ...
            if submit or skip:
                ...
                if not user_answer.strip() and not skip:
                    ...
                    with st.spinner("🤖 Scoring your answer..."):
                        score = score_answer(user_answer, question_data["answer"]) if user_answer.strip() else 0.0
                        feedback = get_feedback(question_data["question"],
                            user_answer if user_answer.strip() else "(skipped)",
                            question_data["answer"], score)
                    save_score(question_data["topic"], question_data["question"], score)
            ...
            label = "Excellent!" if score >= 7 else "Good effort!" if score >= 5 else "Keep practicing!"
            ...
            <div style="background:{color}22; border:2px solid {color};
                ...
                <p style="color:{color}; margin:5px 0 0 0; font-size:18px;">{label}</p>
            </div>
            """, unsafe_allow_html=True)
            ...
            with c1:
                if st.button("🔊 Hear Feedback", key="speak_fb"):
                    ...
                    st_html(f"<script>window.speechSynthesis.cancel(); var u=new SpeechSynthesisUtterance(`{clean}`); u.lang='en-US'; u.rate=0.85; window.speechSynthesis.speak(u);</script>", height=0)
            with c2:
                if st.button("⏹️ Stop", key="stop_fb"):
                    st_html("<script>window.speechSynthesis.cancel();</script>", height=0)
        ...
        if st.session_state.pending_speech and st.session_state.auto_speak:
            ...
            st.session_state.pending_speech = ""
            st_html(f"""
            <div style="background:#e8f5e9; padding:8px; border-radius:8px; font-size:13px; margin:5px 0;">
                🔊 <b>Speaking feedback...</b>
                <button onclick="window.speechSynthesis.cancel()" style="margin-left:8px; background:#ff4b4b; color:white; border:none; padding:3px 10px; border-radius:4px; cursor:pointer;">⏹️ Stop</button>
            </div>
            <script>
            setTimeout(function() {{
                window.speechSynthesis.cancel();
        ...
        import pandas as pd
        report = get_final_report(st.session_state.questions, st.session_state.scores)
        ...
        st.markdown(f"""
        <div style="background:{color}22; border:3px solid {color}; border-radius:20px; padding:30px; text-align:center; margin-bottom:25px;">
            <h1 style="color:{color}; font-size:3rem; margin:0;">{report['total_score']}/{report['max_score']}</h1>
        ...
        cols = st.columns(10)
        for i, (s, q) in enumerate(zip(st.session_state.scores, st.session_state.questions)):
            c = "#28a745" if s >= 7 else "#ffc107" if s >= 5 else "#dc3545"
            cols[i].markdown(
                f"<div style='text-align:center; background:{c}22; border:1px solid {c}; border-radius:8px; padding:8px;'>"
                f"<b style='color:{c}'>Q{i+1}</b><br><span style='color:{c}; font-size:18px;'>{s}</span><br>"
                f"<span style='font-size:10px; color:#888;'>{q['topic'][:6]}</span></div>",
                unsafe_allow_html=True)
        ...
        with col1:
            ...
            if report["strong_topics"]:
                for t in report["strong_topics"]:
                    st.success(f"✅ {t} — {report['topic_averages'].get(t,0)}/10")
            else:
        ...
        with col2:
            ...
            if report["weak_topics"]:
                for t in report["weak_topics"]:
                    st.error(f"⚠️ {t} — {report['topic_averages'].get(t,0)}/10")
            else:
        ...
        with col3:
            ...
            st.download_button("📥 Download Report", score_df.to_csv(index=False),
                               "quiz_report.csv", "text/csv", use_container_width=True)

    st.stop()


# ══════════════════════════════════════════════════
# SCREEN — IDEA GENERATOR MODE
# ══════════════════════════════════════════════════
...
# ══════════════════════════════════════════════════
# SCREEN — COURSE MODULE IDEAS MODE  ← NEW
# ══════════════════════════════════════════════════
if st.session_state.mode == "course":
    from course_module import show_course_module
    show_course_module()
    st.stop()
...
with st.sidebar:
    ...
    input_mode = st.radio("How do you want to ask?", ["⌨️ Type", "🎤 Speak"], index=0)
    st.session_state.input_mode = "speak" if "Speak" in input_mode else "type"
    st.divider()
    st.subheader("🔊 Bot Voice")
    ...
    st.divider()
    st.subheader("📚 Topics")
    for t in ["Machine Learning", "Deep Learning", "NLP", "Statistics", "Python & SQL", "System Design"]:
        st.write(f"✅ {t}")
    st.divider()
    ...
    if st.button("🎯 Switch to Quiz", use_container_width=True, type="primary"):
        st.session_state.mode = "quiz"
        st.rerun()
    st.divider()
    if st.button("🏠 Back to Home", use_container_width=True):
...
c1, c2 = st.columns(2)
with c1:
    st.info(f"🎤 {'Speech Mode' if st.session_state.input_mode == 'speak' else 'Text Mode'}")
with c2:
        ...
        if msg["role"] == "assistant":
            ...
            with c1:
                if st.button("🔊 Speak", key=f"sp_{i}"):
                    ...
                    st_html(f"<script>window.speechSynthesis.cancel(); var u=new SpeechSynthesisUtterance(`{clean}`); u.lang='en-US'; u.rate=0.9; window.speechSynthesis.speak(u);</script>", height=0)
            with c2:
...
def process_question(question: str):
    ...
    with st.chat_message("assistant"):
        ...
        c1, c2 = st.columns(2)
        with c1:
            if st.button("🔊 Speak Answer", key=f"spk_{len(st.session_state.messages)}"):
                clean = answer.replace('"', "'").replace('\n', ' ').replace('`', '')[:3000]
                st_html(f"<script>window.speechSynthesis.cancel(); var u=new SpeechSynthesisUtterance(`{clean}`); u.lang='en-US'; u.rate=0.9; window.speechSynthesis.speak(u);</script>", height=0)
        with c2:
            if st.button("⏹️ Stop", key=f"stp_{len(st.session_state.messages)}"):
                st_html("<script>window.speechSynthesis.cancel();</script>", height=0)
...
if st.session_state.input_mode == "speak":
    ...
    speech_html = """
    <div style="font-family:Arial; padding:10px; text-align:center;">
        <button id="mic-btn" onclick="toggleMic()" style="background:#ff4b4b; color:white; border:none; padding:14px 28px; border-radius:50px; font-size:18px; cursor:pointer; width:240px; margin:6px;">🎤 Start Speaking</button>
        <div id="status-box" style="background:#f0f2f6; border-radius:8px; padding:10px; font-size:14px; color:#555; margin:10px auto; max-width:560px;">Click mic to begin</div>
        <div style="text-align:left; max-width:560px; margin:0 auto 6px auto; font-size:13px; color:#888;">📝 Recognized text:</div>
        <div id="transcript-box" style="background:white; border:2px solid #ddd; border-radius:10px; padding:14px; font-size:16px; font-weight:bold; color:#333; min-height:60px; max-width:560px; margin:0 auto 14px auto; text-align:left; white-space:pre-wrap;">(nothing heard yet)</div>
        <button onclick="useThisText()" style="background:#28a745; color:white; border:none; padding:14px 28px; border-radius:50px; font-size:18px; cursor:pointer; width:240px; margin:6px;">✅ Use This Text</button>
    </div>
    <script>
    ...
    function startMic(){
        ...
        recognition=new API(); recognition.lang='en-US'; recognition.continuous=true; recognition.interimResults=true;
        ...
        recognition.onresult=function(e){ finalText=''; var interim='';
            for(var i=0;i<e.results.length;i++){
                if(e.results[i].isFinal) finalText+=e.results[i][0].transcript+' ';
                else interim+=e.results[i][0].transcript;
            }
            document.getElementById('transcript-box').innerText=(finalText+interim).trim();
        };
        recognition.onerror=function(e){ isListening=false; resetMic(); setStatus('❌ Error: '+e.error,'#c62828','#ffebee'); };
        recognition.onend=function(){ isListening=false; resetMic();
            if(finalText.trim()) setStatus('✅ Done! Click "Use This Text"','#1b5e20','#e8f5e9');
        };
        recognition.start();
    }
    function stopMic(){if(recognition)recognition.stop();isListening=false;resetMic();}
    function resetMic(){ document.getElementById('mic-btn').style.background='#ff4b4b'; document.getElementById('mic-btn').innerText='🎤 Start Speaking'; }
    function setStatus(msg,color,bg){ var b=document.getElementById('status-box'); b.innerText=msg; b.style.color=color; b.style.background=bg; }
    function useThisText(){
        var text=finalText.trim()||document.getElementById('transcript-box').innerText.trim();
        if(!text||text==='(nothing heard yet)'||text==='...'||text.length<2){ setStatus('❌ No speech. Click mic first!','#c62828','#ffebee'); return; }
        ...
        setTimeout(function(){
            try {
                ...
                if(filled){ setStatus('✅ Question box filled! Edit if needed, then Get Answer.','#1b5e20','#e8f5e9'); }
                else{ setStatus('⚠️ Could not find question box. Please type manually.','#c62828','#ffebee'); }
            } catch(err){ setStatus('⚠️ Could not fill automatically. Please type manually.','#c62828','#ffebee'); }
        }, 150);
    }
    </script>
    ...
    question_input = st.text_area(label="q", value=st.session_state.recognized_text,
        height=110, placeholder="Speak above → click 'Use This Text' → appears here. Or type directly.",
        label_visibility="collapsed", key="speech_question_area")
    col1, col2 = st.columns([3, 1])
    with col1:
        get_ans = st.button("🚀 Get Answer", type="primary", use_container_width=True, key="speech_get_answer")
    with col2:
...
if st.session_state.pending_speech and st.session_state.auto_speak:
    ...
    st.session_state.pending_speech = ""
    st_html(f"""
    <div style="background:#e8f5e9; padding:10px; border-radius:8px; margin:5px 0; font-size:14px;">
        🔊 <b>Speaking answer...</b>
        <button onclick="window.speechSynthesis.cancel()" style="margin-left:10px; background:#ff4b4b; color:white; border:none; padding:4px 12px; border-radius:5px; cursor:pointer;">⏹️ Stop</button>
    </div>
    <script>
    setTimeout(function() {{
        window.speechSynthesis.cancel();
